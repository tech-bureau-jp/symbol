...
class StructFormatter(AbstractTypeFormatter):
    ...
    def non_reserved_fields(self, include_inherited=True):
        fields = filter_size_if_first(
            filterfalse(
                is_computed,
                filterfalse(
                    is_bound_size, filterfalse(is_reserved, self.non_const_fields())
                ),
            )
        )
        return self._filter_inherited_fields(fields, include_inherited)
    ...
    def _is_inherited_field(self, field):
        ...
        return bool(
            next(
                (
                    base_struct_field
                    for base_struct_field in self.base_struct.fields
                    if field.name == base_struct_field.name
                ),
                None,
            )
        )
    ...
    def generate_type_hints(self):
        ...
        for field in self.non_reserved_fields(include_inherited=False):
            ...
            hints.append(
                f"'{field.extensions.printer.name}': '{field.extensions.printer.type_hint}'"
            )
    ...
    def get_fields(self):
        return list(map(self.generate_class_field, self.const_fields())) + [
            ...
            if const_field:
                ...
                if field.is_conditional:
                    ...
                    condition_field = next(
                        f
                        for f in self.non_const_fields()
                        if condition_field_name == f.name
                    )
                    condition_model = condition_field.extensions.type_model

                    # only initialize default implicit union field in constructor
                    if (
                        f"{condition_model.name}.{conditional.value}"
                        != condition_field.extensions.printer.get_default_value()
                    ):
                        value = "None"
    ...
    def get_comparer_descriptor(self):
        ...
        body = ""
        if any(
            "ripemd_keccak_256" == transform for (_, transform) in self.struct.comparer
        ):
    ...
    def generate_condition(self, field, prefix_field=False):
        ...
        condition_field = next(
            f for f in self.non_const_fields() if condition_field_name == f.name
        )
        ...
        yoda_value = (
            ...
            else f"{condition_model.name}.{value}"
        )
        ...
        field_postfix = (
            "_computed" if prefix_field and is_computed(condition_field) else ""
        )
    ...
    def generate_deserialize_field(self, field, arg_buffer_name=None):
        ...
        if size_fields:
            assert len(size_fields) == 1, (
                f"unexpected number of size_fields associated with {field.name}"
            )
            buffer_load_name = f"buffer[:{size_fields[0].name}]"

        use_custom_buffer_name = arg_buffer_name or size_fields
        load = (
            field.extensions.printer.load(buffer_load_name)
            if use_custom_buffer_name
            else field.extensions.printer.load()
        )
            ...
            if field.is_conditional:
                ...
                if condition_field_name not in processed_fields:
                    if condition_field_name not in queued_fields:
                        ...
                        temporary_buffer = create_temporary_buffer_name(
        ...
        if is_bound_size(field):
            ...
            if bound_field.display_type.is_array:
                if (
                    ...
                    or not bound_field.field_type.is_byte_constrained
                ):
                    ...
                    if condition and bound_condition:
                        raise RuntimeError(
                            "do not know yet how to generate both conditions"
                        )
    ...
    def get_size_descriptor(self):
        ...
        body += "".join(
            map(
                self.generate_size_field, self.non_const_fields(include_inherited=False)
            )
        )
        body += "return size"
        return MethodDescriptor(body=body)
    ...
    def get_getter_descriptors(self):
        return list(
            map(
                self.create_getter_descriptor,
                self.non_reserved_fields(include_inherited=False),
            )
        ) + (
            list(
                map(
                    self.create_getter_descriptor,
                    self.computed_fields(include_inherited=False),
                )
            )
        )
    ...
    def generate_str_field(self, field):
        ...
        field_to_string = (
            field_to_string if "{" in field_to_string else f"{{{field_to_string}}}"
        )
        return indent_if_conditional(
            condition,
            f"result += f'{printable_field_name(field)}: {field_to_string}, '\n",
        )
    ...
    def generate_json_field(self, field):
        ...
        return indent_if_conditional(
            condition, f"result['{printable_field_name(field)}'] = {field_json_value}\n"
        )
