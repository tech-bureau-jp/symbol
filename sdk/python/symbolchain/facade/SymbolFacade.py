...
class SymbolAccount(SymbolPublicAccount):
    ...
    def cosign_transaction_hash(self, transaction_hash, detached=False):
        ...
        return self._facade.cosign_transaction_hash(
            self.key_pair, transaction_hash, detached
        )
...
class SymbolFacade:
    ...
    def __init__(self, network, account_descriptor_repository=None):
        ...
        self.network = (
            NetworkLocator.find_by_name(Network.NETWORKS, network)
    ...
    def _create_symbol_transaction_factory(self):
        ...
        if self.account_descriptor_repository:
            ...
            type_parsing_rules = (
                self.account_descriptor_repository.to_type_parsing_rules_map(
                    type_to_property_mapping
                )
            )
    ...
    def verify_transaction(self, transaction, signature):
        ...
        return Verifier(transaction.signer_public_key).verify(
            self.extract_signing_payload(transaction), signature
        )
    ...
    def cosign_transaction_hash(key_pair, transaction_hash, detached=False):
        ...
        cosignature.signature = sc.Signature(
            key_pair.sign(transaction_hash.bytes).bytes
        )
        return cosignature
    ...
    def hash_embedded_transactions(embedded_transactions):
        ...
        for embedded_transaction in embedded_transactions:
            hash_builder.update(
                Hash256(hashlib.sha3_256(embedded_transaction.serialize()).digest())
            )
    ...
    def _is_aggregate_transaction(transaction_buffer):
        transaction_type_offset = (
            TRANSACTION_HEADER_SIZE + 2
        )  # skip version and network byte
        transaction_type = (
            transaction_buffer[transaction_type_offset + 1] << 8
        ) + transaction_buffer[transaction_type_offset]
        aggregate_types = [
