...
class Transaction:
    ...
    def _deserialize(cls, buffer: memoryview, instance) -> (int, int):
        ...
        entity_body_reserved_1 = int.from_bytes(
            buffer[:2], byteorder="little", signed=False
        )
        buffer = buffer[2:]
        ...
        signer_public_key_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert signer_public_key_size == 32, (
            f"Invalid value of reserved field ({signer_public_key_size})"
        )
        ...
        assert signature_size == 64, (
            f"Invalid value of reserved field ({signature_size})"
        )
    ...
    def _serialize(self, buffer: memoryview):
        ...
        buffer += self._signer_public_key_size.to_bytes(
...
class AccountKeyLinkTransactionV1(Transaction):
    ...
    def deserialize(
        ...
        remote_public_key_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert remote_public_key_size == 32, (
            f"Invalid value of reserved field ({remote_public_key_size})"
        )
    ...
    def serialize(self) -> bytes:
        ...
        buffer += self._remote_public_key_size.to_bytes(
            4, byteorder="little", signed=False
        )
...
class NonVerifiableAccountKeyLinkTransactionV1(NonVerifiableTransaction):
    ...
    def deserialize(
        ...
        (window_start, window_end) = NonVerifiableTransaction._deserialize(
            buffer, instance
        )
        buffer = buffer[window_start:window_end]
        link_action = LinkAction.deserialize(buffer)
...
class MosaicId:
    TYPE_HINTS = {"namespace_id": "struct:NamespaceId", "name": "bytes_array"}
...
class Mosaic:
    TYPE_HINTS = {"mosaic_id": "struct:MosaicId", "amount": "pod:Amount"}
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        buffer += self.mosaic_id.size.to_bytes(
            4, byteorder="little", signed=False
        )  # mosaic_id_size
...
class SizePrefixedMosaic:
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        buffer += self.mosaic.size.to_bytes(
            4, byteorder="little", signed=False
        )  # mosaic_size
...
class MosaicLevy:
    ...
    def deserialize(cls, payload: bytes | bytearray | memoryview) -> MosaicLevy:
        ...
        recipient_address_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert recipient_address_size == 40, (
            f"Invalid value of reserved field ({recipient_address_size})"
        )
    ...
    def serialize(self) -> bytes:
        ...
        buffer += self._recipient_address_size.to_bytes(
            4, byteorder="little", signed=False
        )
...
class SizePrefixedMosaicProperty:
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        buffer += self.property_.size.to_bytes(
            4, byteorder="little", signed=False
        )  # property_size
...
class MosaicDefinition:
    ...
    def deserialize(cls, payload: bytes | bytearray | memoryview) -> MosaicDefinition:
        ...
        owner_public_key_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert owner_public_key_size == 32, (
            f"Invalid value of reserved field ({owner_public_key_size})"
        )
        ...
        buffer = buffer[4:]
        properties = ArrayHelpers.read_array_count(
            buffer, SizePrefixedMosaicProperty, properties_count
        )
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        buffer += self._owner_public_key_size.to_bytes(
            4, byteorder="little", signed=False
        )
        ...
        buffer += len(self._description).to_bytes(
            4, byteorder="little", signed=False
        )  # description_size
        buffer += self._description
        buffer += len(self._properties).to_bytes(
            4, byteorder="little", signed=False
        )  # properties_count
...
class MosaicDefinitionTransactionV1(Transaction):
    ...
    def deserialize(
        ...
        (window_start, window_end) = Transaction._deserialize(buffer, instance)
        buffer = buffer[window_start:window_end]
        mosaic_definition_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        # marking sizeof field
        mosaic_definition = MosaicDefinition.deserialize(
            buffer[:mosaic_definition_size]
        )
        buffer = buffer[mosaic_definition.size :]
        rental_fee_sink_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert rental_fee_sink_size == 40, (
            f"Invalid value of reserved field ({rental_fee_sink_size})"
        )
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        super()._serialize(buffer)
        buffer += self.mosaic_definition.size.to_bytes(
            4, byteorder="little", signed=False
        )  # mosaic_definition_size
        buffer += self._mosaic_definition.serialize()
        buffer += self._rental_fee_sink_size.to_bytes(
            4, byteorder="little", signed=False
        )
...
class MultisigAccountModification:
    ...
    def comparer(self) -> tuple:
        ...
        return (
            ...
            if not isinstance(self.modification_type, Enum)
            else self.modification_type.value,
    ...
    def deserialize(
        ...
        cosignatory_public_key_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert cosignatory_public_key_size == 32, (
            f"Invalid value of reserved field ({cosignatory_public_key_size})"
        )
    ...
    def serialize(self) -> bytes:
        ...
        buffer += self._cosignatory_public_key_size.to_bytes(
            4, byteorder="little", signed=False
        )
...
class SizePrefixedMultisigAccountModification:
    ...
    def deserialize(
        ...
        buffer = buffer[4:]
        # marking sizeof field
        modification = MultisigAccountModification.deserialize(
            buffer[:modification_size]
        )
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        buffer += self.modification.size.to_bytes(
            4, byteorder="little", signed=False
        )  # modification_size
...
class MultisigAccountModificationTransactionV1(Transaction):
    ...
    def sort(self) -> None:
        self._modifications = sorted(
            self._modifications,
            key=lambda e: e.modification.comparer()
            if hasattr(e.modification, "comparer")
            else e.modification,
        )
    ...
    def deserialize(
        ...
        (window_start, window_end) = Transaction._deserialize(buffer, instance)
        buffer = buffer[window_start:window_end]
        modifications_count = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        modifications = ArrayHelpers.read_array_count(
            buffer,
            SizePrefixedMultisigAccountModification,
            modifications_count,
            lambda e: e.modification.comparer()
            if hasattr(e.modification, "comparer")
            else e.modification,
        )
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        super()._serialize(buffer)
        buffer += len(self._modifications).to_bytes(
            4, byteorder="little", signed=False
        )  # modifications_count
...
class MultisigAccountModificationTransactionV2(Transaction):
    ...
    def deserialize(
        ...
        buffer = buffer[ArrayHelpers.size(modifications) :]
        min_approval_delta_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert min_approval_delta_size == 4, (
            f"Invalid value of reserved field ({min_approval_delta_size})"
        )
    ...
    def serialize(self) -> bytes:
        ...
        buffer += self._min_approval_delta_size.to_bytes(
            4, byteorder="little", signed=False
        )
...
class CosignatureV1Body:
    ...
    def deserialize(cls, payload: bytes | bytearray | memoryview) -> CosignatureV1Body:
        ...
        other_transaction_hash_outer_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert other_transaction_hash_outer_size == 36, (
            f"Invalid value of reserved field ({other_transaction_hash_outer_size})"
        )
        other_transaction_hash_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert other_transaction_hash_size == 32, (
            f"Invalid value of reserved field ({other_transaction_hash_size})"
        )
        other_transaction_hash = Hash256.deserialize(buffer)
        buffer = buffer[other_transaction_hash.size :]
        multisig_account_address_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert multisig_account_address_size == 40, (
            f"Invalid value of reserved field ({multisig_account_address_size})"
        )
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        buffer += self._other_transaction_hash_outer_size.to_bytes(
            4, byteorder="little", signed=False
        )
        buffer += self._other_transaction_hash_size.to_bytes(
            4, byteorder="little", signed=False
        )
        buffer += self._other_transaction_hash.serialize()
        buffer += self._multisig_account_address_size.to_bytes(
            4, byteorder="little", signed=False
        )
    ...
    def __str__(self) -> str:
        ...
        result += (
            f"multisig_account_address: {self._multisig_account_address.__str__()}, "
        )
        result += ")"
        return result
...
class SizePrefixedCosignatureV1:
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        buffer += self.cosignature.size.to_bytes(
            4, byteorder="little", signed=False
        )  # cosignature_size
...
class MultisigTransactionV1(Transaction):
    ...
    def deserialize(
        ...
        (window_start, window_end) = Transaction._deserialize(buffer, instance)
        buffer = buffer[window_start:window_end]
        inner_transaction_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        # marking sizeof field
        inner_transaction = NonVerifiableTransactionFactory.deserialize(
            buffer[:inner_transaction_size]
        )
        buffer = buffer[inner_transaction.size :]
        cosignatures_count = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        cosignatures = ArrayHelpers.read_array_count(
            buffer, SizePrefixedCosignatureV1, cosignatures_count
        )
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        super()._serialize(buffer)
        buffer += self.inner_transaction.size.to_bytes(
            4, byteorder="little", signed=False
        )  # inner_transaction_size
        buffer += self._inner_transaction.serialize()
        buffer += len(self._cosignatures).to_bytes(
            4, byteorder="little", signed=False
        )  # cosignatures_count
...
class NamespaceRegistrationTransactionV1(Transaction):
    ...
    def serialize(self) -> bytes:
        ...
        buffer += self._name
        buffer += (
            len(self._parent_name) if self._parent_name is not None else 4294967295
        ).to_bytes(4, byteorder="little", signed=False)  # parent_name_size
        if self.parent_name:
...
class Message:
    TYPE_HINTS = {"message_type": "enum:MessageType", "message": "bytes_array"}
...
class TransferTransactionV1(Transaction):
    ...
    def deserialize(
        ...
        amount = Amount.deserialize(buffer)
        buffer = buffer[amount.size :]
        message_envelope_size = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        message = None
    ...
    def serialize(self) -> bytes:
        ...
        buffer += self._recipient_address.serialize()
        buffer += self._amount.serialize()
        buffer += self.message_envelope_size_computed.to_bytes(
            4, byteorder="little", signed=False
        )
        if 0 != self.message_envelope_size_computed:
...
class TransferTransactionV2(Transaction):
    ...
    def deserialize(
        ...
        mosaics = ArrayHelpers.read_array_count(
            buffer, SizePrefixedMosaic, mosaics_count
        )
...
class TransactionFactory:
    ...
    def deserialize(cls, payload: bytes) -> Transaction:
        ...
        mapping = {
            ...
            (
                CosignatureV1.TRANSACTION_TYPE,
