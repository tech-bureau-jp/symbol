...
class BlockDuration(BaseValue):
    ...
    def deserialize(cls, payload: bytes | bytearray | memoryview) -> BlockDuration:
        ...
        return BlockDuration(
            int.from_bytes(buffer[:8], byteorder="little", signed=False)
        )
...
class BlockFeeMultiplier(BaseValue):
    ...
    def deserialize(cls, payload: bytes | bytearray | memoryview) -> BlockFeeMultiplier:
        ...
        return BlockFeeMultiplier(
            int.from_bytes(buffer[:4], byteorder="little", signed=False)
        )
...
class Mosaic:
    TYPE_HINTS = {"mosaic_id": "pod:MosaicId", "amount": "pod:Amount"}
...
class UnresolvedMosaic:
    TYPE_HINTS = {"mosaic_id": "pod:UnresolvedMosaicId", "amount": "pod:Amount"}
...
class TransactionType(Enum):
    ...
    def deserialize(cls, payload: bytes | bytearray | memoryview) -> TransactionType:
        ...
        return TransactionType(
            int.from_bytes(buffer[:2], byteorder="little", signed=False)
        )
...
class Transaction:
    ...
    def _deserialize(cls, buffer: memoryview, instance) -> (int, int):
        ...
        verifiable_entity_header_reserved_1 = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert verifiable_entity_header_reserved_1 == 0, (
            f"Invalid value of reserved field ({verifiable_entity_header_reserved_1})"
        )
        ...
        entity_body_reserved_1 = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert entity_body_reserved_1 == 0, (
            f"Invalid value of reserved field ({entity_body_reserved_1})"
        )
    ...
    def _serialize(self, buffer: memoryview):
        ...
        buffer += self._verifiable_entity_header_reserved_1.to_bytes(
            4, byteorder="little", signed=False
        )
        buffer += self._signature.serialize()
        buffer += self._signer_public_key.serialize()
        buffer += self._entity_body_reserved_1.to_bytes(
            4, byteorder="little", signed=False
        )
...
class EmbeddedTransaction:
    ...
    def _deserialize(cls, buffer: memoryview, instance) -> (int, int):
        size_ = int.from_bytes(buffer[:4], byteorder="little", signed=False)
        buffer = buffer[4:size_]
        embedded_transaction_header_reserved_1 = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert embedded_transaction_header_reserved_1 == 0, (
            f"Invalid value of reserved field ({embedded_transaction_header_reserved_1})"
        )
        signer_public_key = PublicKey.deserialize(buffer)
        buffer = buffer[signer_public_key.size :]
        entity_body_reserved_1 = int.from_bytes(
    ...
    def _serialize(self, buffer: memoryview):
        buffer += self.size.to_bytes(4, byteorder="little", signed=False)
        buffer += self._embedded_transaction_header_reserved_1.to_bytes(
            4, byteorder="little", signed=False
        )
        buffer += self._signer_public_key.serialize()
        buffer += self._entity_body_reserved_1.to_bytes(
...
class NemesisBlockV1(Block):
    ...
    def deserialize(cls, payload: bytes | bytearray | memoryview) -> NemesisBlockV1:
        ...
        voting_eligible_accounts_count = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        harvesting_eligible_accounts_count = int.from_bytes(
            buffer[:8], byteorder="little", signed=False
        )
        buffer = buffer[8:]
        ...
        transactions = ArrayHelpers.read_variable_size_elements(
            buffer, TransactionFactory, 8, skip_last_element_padding=True
        )
        buffer = buffer[
            ArrayHelpers.size(transactions, 8, skip_last_element_padding=True) :
        ]

        # pylint: disable=protected-access
        instance._voting_eligible_accounts_count = voting_eligible_accounts_count
        instance._harvesting_eligible_accounts_count = (
    ...
    def serialize(self) -> bytes:
        ...
        buffer += self._voting_eligible_accounts_count.to_bytes(
            4, byteorder="little", signed=False
        )
        buffer += self._harvesting_eligible_accounts_count.to_bytes(
        ...
        buffer += ArrayHelpers.write_variable_size_elements(
            self._transactions, 8, skip_last_element_padding=True
        )
        return buffer
    ...
    def to_json(self):
        ...
        result["harvesting_eligible_accounts_count"] = str(
            self._harvesting_eligible_accounts_count
        )
        result["total_voting_balance"] = self._total_voting_balance.to_json()
        result["previous_importance_block_hash"] = (
            self._previous_importance_block_hash.to_json()
        )
...
class NormalBlockV1(Block):
    ...
    TYPE_HINTS = {**Block.TYPE_HINTS, "transactions": "array[Transaction]"}
    ...
    def deserialize(cls, payload: bytes | bytearray | memoryview) -> NormalBlockV1:
        ...
        (window_start, window_end) = Block._deserialize(buffer, instance)
        buffer = buffer[window_start:window_end]
        block_header_reserved_1 = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert block_header_reserved_1 == 0, (
            f"Invalid value of reserved field ({block_header_reserved_1})"
        )
        transactions = ArrayHelpers.read_variable_size_elements(
            buffer, TransactionFactory, 8, skip_last_element_padding=True
        )
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        super()._serialize(buffer)
        buffer += self._block_header_reserved_1.to_bytes(
            4, byteorder="little", signed=False
        )
        buffer += ArrayHelpers.write_variable_size_elements(
            self._transactions, 8, skip_last_element_padding=True
...
class InflationReceipt(Receipt):
    ...
    TYPE_HINTS = {**Receipt.TYPE_HINTS, "mosaic": "struct:Mosaic"}
...
class MosaicExpiredReceipt(Receipt):
    ...
    TYPE_HINTS = {**Receipt.TYPE_HINTS, "artifact_id": "pod:MosaicId"}
...
class NamespaceRegistrationType(Enum):
    ...
    def deserialize(
        ...
        return NamespaceRegistrationType(
            int.from_bytes(buffer[:1], byteorder="little", signed=False)
        )
...
class NamespaceExpiredReceipt(Receipt):
    ...
    TYPE_HINTS = {**Receipt.TYPE_HINTS, "artifact_id": "pod:NamespaceId"}
...
class AddressResolutionEntry:
    TYPE_HINTS = {"source": "struct:ReceiptSource", "resolved_value": "pod:Address"}
...
class AddressResolutionStatement:
    ...
    def deserialize(
        ...
        resolution_entries_count = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        resolution_entries = ArrayHelpers.read_array_count(
            buffer, AddressResolutionEntry, resolution_entries_count
        )
    ...
    def serialize(self) -> bytes:
        ...
        buffer += len(self._resolution_entries).to_bytes(
            4, byteorder="little", signed=False
...
class MosaicResolutionEntry:
    TYPE_HINTS = {"source": "struct:ReceiptSource", "resolved_value": "pod:MosaicId"}
...
class MosaicResolutionStatement:
    ...
    def deserialize(
        ...
        buffer = buffer[4:]
        resolution_entries = ArrayHelpers.read_array_count(
            buffer, MosaicResolutionEntry, resolution_entries_count
        )
        buffer = buffer[ArrayHelpers.size(resolution_entries) :]
...
class TransactionStatement:
    ...
    def serialize(self) -> bytes:
        ...
        buffer += self._primary_id.to_bytes(4, byteorder="little", signed=False)
        buffer += self._secondary_id.to_bytes(4, byteorder="little", signed=False)
        buffer += len(self._receipts).to_bytes(
            4, byteorder="little", signed=False
        )  # receipt_count
...
class BlockStatement:
    ...
    def deserialize(cls, payload: bytes | bytearray | memoryview) -> BlockStatement:
        ...
        transaction_statement_count = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        transaction_statements = ArrayHelpers.read_array_count(
            buffer, TransactionStatement, transaction_statement_count
        )
        buffer = buffer[ArrayHelpers.size(transaction_statements) :]
        address_resolution_statement_count = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        address_resolution_statements = ArrayHelpers.read_array_count(
            buffer, AddressResolutionStatement, address_resolution_statement_count
        )
        buffer = buffer[ArrayHelpers.size(address_resolution_statements) :]
        mosaic_resolution_statement_count = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        mosaic_resolution_statements = ArrayHelpers.read_array_count(
            buffer, MosaicResolutionStatement, mosaic_resolution_statement_count
        )
    ...
    def serialize(self) -> bytes:
        buffer = bytearray()
        buffer += len(self._transaction_statements).to_bytes(
            4, byteorder="little", signed=False
        )  # transaction_statement_count
        buffer += ArrayHelpers.write_array(self._transaction_statements)
        buffer += len(self._address_resolution_statements).to_bytes(
            4, byteorder="little", signed=False
        )  # address_resolution_statement_count
        buffer += ArrayHelpers.write_array(self._address_resolution_statements)
        buffer += len(self._mosaic_resolution_statements).to_bytes(
            4, byteorder="little", signed=False
        )  # mosaic_resolution_statement_count
    ...
    def __str__(self) -> str:
        result = "("
        result += (
            f"transaction_statements: {list(map(str, self._transaction_statements))}, "
        )
    ...
    def to_json(self):
        ...
        result["transaction_statements"] = [
            e.to_json() for e in self._transaction_statements
        ]
        result["address_resolution_statements"] = [
            e.to_json() for e in self._address_resolution_statements
        ]
        result["mosaic_resolution_statements"] = [
            e.to_json() for e in self._mosaic_resolution_statements
        ]
        return result
...
class Cosignature:
    TYPE_HINTS = {"signer_public_key": "pod:PublicKey", "signature": "pod:Signature"}
...
class AggregateCompleteTransactionV1(Transaction):
    ...
    def deserialize(
        ...
        buffer = buffer[4:]
        aggregate_transaction_header_reserved_1 = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert aggregate_transaction_header_reserved_1 == 0, (
            f"Invalid value of reserved field ({aggregate_transaction_header_reserved_1})"
        )
        transactions = ArrayHelpers.read_variable_size_elements(
            buffer[:payload_size],
            EmbeddedTransactionFactory,
            8,
            skip_last_element_padding=False,
        )
    ...
    def serialize(self) -> bytes:
        ...
        super()._serialize(buffer)
        buffer += self._transactions_hash.serialize()
        buffer += ArrayHelpers.size(
            self.transactions, 8, skip_last_element_padding=False
        ).to_bytes(4, byteorder="little", signed=False)  # payload_size
        buffer += self._aggregate_transaction_header_reserved_1.to_bytes(
            4, byteorder="little", signed=False
        )
        buffer += ArrayHelpers.write_variable_size_elements(
            self._transactions, 8, skip_last_element_padding=False
        )
...
class SecretProofTransactionV1(Transaction):
    ...
    def serialize(self) -> bytes:
        ...
        buffer += self._recipient_address.serialize()
        buffer += self._secret.serialize()
        buffer += len(self._proof).to_bytes(
...
class AccountMetadataTransactionV1(Transaction):
    ...
    def deserialize(
        ...
        buffer = buffer[target_address.size :]
        scoped_metadata_key = int.from_bytes(
            buffer[:8], byteorder="little", signed=False
        )
        buffer = buffer[8:]
    ...
    def serialize(self) -> bytes:
        ...
        super()._serialize(buffer)
        buffer += self._target_address.serialize()
        buffer += self._scoped_metadata_key.to_bytes(
            8, byteorder="little", signed=False
        )
        buffer += self._value_size_delta.to_bytes(2, byteorder="little", signed=True)
        buffer += len(self._value).to_bytes(
            2, byteorder="little", signed=False
        )  # value_size
        buffer += self._value
        return buffer
...
class MultisigAccountModificationTransactionV1(Transaction):
    ...
    def __init__(self):
        ...
        self._multisig_account_modification_transaction_body_reserved_1 = (
            0  # reserved field
        )
    ...
    def deserialize(
        ...
        buffer = buffer[1:]
        address_additions_count = int.from_bytes(
            buffer[:1], byteorder="little", signed=False
        )
        buffer = buffer[1:]
        address_deletions_count = int.from_bytes(
            buffer[:1], byteorder="little", signed=False
        )
        buffer = buffer[1:]
        multisig_account_modification_transaction_body_reserved_1 = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert multisig_account_modification_transaction_body_reserved_1 == 0, (
            f"Invalid value of reserved field ({multisig_account_modification_transaction_body_reserved_1})"
        )
        address_additions = ArrayHelpers.read_array_count(
            buffer, UnresolvedAddress, address_additions_count
        )
        buffer = buffer[ArrayHelpers.size(address_additions) :]
        address_deletions = ArrayHelpers.read_array_count(
            buffer, UnresolvedAddress, address_deletions_count
        )
    ...
    def serialize(self) -> bytes:
        ...
        buffer += len(self._address_additions).to_bytes(
            1, byteorder="little", signed=False
        )  # address_additions_count
        buffer += len(self._address_deletions).to_bytes(
            1, byteorder="little", signed=False
        )  # address_deletions_count
        buffer += (
            self._multisig_account_modification_transaction_body_reserved_1.to_bytes(
                4, byteorder="little", signed=False
            )
        )
...
class NamespaceRegistrationTransactionV1(Transaction):
    ...
    def serialize(self) -> bytes:
        ...
        buffer += len(self._name).to_bytes(
            1, byteorder="little", signed=False
        )  # name_size
        buffer += self._name
        return buffer
...
class AccountAddressRestrictionTransactionV1(Transaction):
    ...
    def deserialize(
        ...
        restriction_additions_count = int.from_bytes(
            buffer[:1], byteorder="little", signed=False
        )
        buffer = buffer[1:]
        restriction_deletions_count = int.from_bytes(
            buffer[:1], byteorder="little", signed=False
        )
        buffer = buffer[1:]
        account_restriction_transaction_body_reserved_1 = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert account_restriction_transaction_body_reserved_1 == 0, (
            f"Invalid value of reserved field ({account_restriction_transaction_body_reserved_1})"
        )
        restriction_additions = ArrayHelpers.read_array_count(
            buffer, UnresolvedAddress, restriction_additions_count
        )
        buffer = buffer[ArrayHelpers.size(restriction_additions) :]
        restriction_deletions = ArrayHelpers.read_array_count(
            buffer, UnresolvedAddress, restriction_deletions_count
        )
    ...
    def serialize(self) -> bytes:
        ...
        buffer += len(self._restriction_additions).to_bytes(
            1, byteorder="little", signed=False
        )  # restriction_additions_count
        buffer += len(self._restriction_deletions).to_bytes(
            1, byteorder="little", signed=False
        )  # restriction_deletions_count
        buffer += self._account_restriction_transaction_body_reserved_1.to_bytes(
            4, byteorder="little", signed=False
        )
    ...
    def __str__(self) -> str:
        ...
        result += (
            f"restriction_additions: {list(map(str, self._restriction_additions))}, "
        )
        result += (
            f"restriction_deletions: {list(map(str, self._restriction_deletions))}, "
        )
        result += ")"
        return result
    ...
    def to_json(self):
        ...
        result["restriction_additions"] = [
            e.to_json() for e in self._restriction_additions
        ]
        result["restriction_deletions"] = [
            e.to_json() for e in self._restriction_deletions
        ]
        return result
...
class AccountMosaicRestrictionTransactionV1(Transaction):
    ...
    def deserialize(
        ...
        restriction_additions = ArrayHelpers.read_array_count(
            buffer, UnresolvedMosaicId, restriction_additions_count
        )
        buffer = buffer[ArrayHelpers.size(restriction_additions) :]
        restriction_deletions = ArrayHelpers.read_array_count(
            buffer, UnresolvedMosaicId, restriction_deletions_count
        )
        buffer = buffer[ArrayHelpers.size(restriction_deletions) :]
...
class AccountOperationRestrictionTransactionV1(Transaction):
    ...
    def deserialize(
        ...
        restriction_additions = ArrayHelpers.read_array_count(
            buffer, TransactionType, restriction_additions_count
        )
        buffer = buffer[ArrayHelpers.size(restriction_additions) :]
        restriction_deletions = ArrayHelpers.read_array_count(
            buffer, TransactionType, restriction_deletions_count
        )
        buffer = buffer[ArrayHelpers.size(restriction_deletions) :]
...
class MosaicAddressRestrictionTransactionV1(Transaction):
    ...
    def deserialize(
        ...
        buffer = buffer[8:]
        previous_restriction_value = int.from_bytes(
            buffer[:8], byteorder="little", signed=False
        )
        buffer = buffer[8:]
        new_restriction_value = int.from_bytes(
            buffer[:8], byteorder="little", signed=False
        )
        buffer = buffer[8:]
        target_address = UnresolvedAddress.deserialize(buffer)
    ...
    def serialize(self) -> bytes:
        ...
        buffer += self._previous_restriction_value.to_bytes(
            8, byteorder="little", signed=False
        )
        buffer += self._new_restriction_value.to_bytes(
            8, byteorder="little", signed=False
        )
        buffer += self._target_address.serialize()
        return buffer
    ...
    def __str__(self) -> str:
        ...
        result += (
            f"previous_restriction_value: 0x{self._previous_restriction_value:X}, "
        )
...
class MosaicGlobalRestrictionTransactionV1(Transaction):
    ...
    def __str__(self) -> str:
        ...
        result += f"new_restriction_value: 0x{self._new_restriction_value:X}, "
        result += (
            f"previous_restriction_type: {self._previous_restriction_type.__str__()}, "
        )
...
class TransferTransactionV1(Transaction):
    ...
    def sort(self) -> None:
        self._mosaics = sorted(
            self._mosaics,
            key=lambda e: e.mosaic_id.comparer()
            if hasattr(e.mosaic_id, "comparer")
            else e.mosaic_id,
        )
    ...
    def deserialize(
        ...
        buffer = buffer[1:]
        transfer_transaction_body_reserved_1 = int.from_bytes(
            buffer[:1], byteorder="little", signed=False
        )
        buffer = buffer[1:]
        assert transfer_transaction_body_reserved_1 == 0, (
            f"Invalid value of reserved field ({transfer_transaction_body_reserved_1})"
        )
        transfer_transaction_body_reserved_2 = int.from_bytes(
            buffer[:4], byteorder="little", signed=False
        )
        buffer = buffer[4:]
        assert transfer_transaction_body_reserved_2 == 0, (
            f"Invalid value of reserved field ({transfer_transaction_body_reserved_2})"
        )
        mosaics = ArrayHelpers.read_array_count(
            buffer,
            UnresolvedMosaic,
            mosaics_count,
            lambda e: e.mosaic_id.comparer()
            if hasattr(e.mosaic_id, "comparer")
            else e.mosaic_id,
        )
    ...
    def serialize(self) -> bytes:
        ...
        super()._serialize(buffer)
        buffer += self._recipient_address.serialize()
        buffer += len(self._message).to_bytes(
            2, byteorder="little", signed=False
        )  # message_size
        buffer += len(self._mosaics).to_bytes(
            1, byteorder="little", signed=False
        )  # mosaics_count
        buffer += self._transfer_transaction_body_reserved_1.to_bytes(
            1, byteorder="little", signed=False
        )
        buffer += self._transfer_transaction_body_reserved_2.to_bytes(
            4, byteorder="little", signed=False
        )
        buffer += ArrayHelpers.write_array(
            self._mosaics,
            lambda e: e.mosaic_id.comparer()
            if hasattr(e.mosaic_id, "comparer")
            else e.mosaic_id,
