...
CosignTransactionTestDescriptor = namedtuple(
    "CosignTransactionTestDescriptor", ["sign_transaction", "cosign_transaction"]
)
...
class SymbolFacadeTest(unittest.TestCase):
        ...
        return facade.transaction_factory.create(
            {
                ...
                "mosaics": [{"mosaic_id": 0x2CF403E85507F39E, "amount": 1000000}],
            }
        )
    ...
    def _assert_can_cosign_transaction(self, test_descriptor, detached=False):
        ...
        cosigner_private_key = PrivateKey(
        ...
        signature = test_descriptor.sign_transaction(
            facade, signer_private_key, transaction
        )
        facade.transaction_factory.attach_signature(transaction, signature)

        # Act:
        cosignature = test_descriptor.cosign_transaction(
            facade, cosigner_private_key, transaction, detached
        )
        ...
        self.assertEqual(
            sc.PublicKey(
                "29856F43A5C4CBDE42F2FAC775A6F915E9E5638CF458E9352E7B410B662473A3"
            ),
            cosignature.signer_public_key,
        )
        self.assertEqual(
            sc.Signature(
                "204BD2C4F86B66313E5C5F817FD650B108826D53EDEFC8BDFF936E4D6AA07E38"
    ...
    def _run_test_can_cosign_transaction_detached(self, test_descriptor):
        ...
        self.assertEqual(
            sc.Hash256(
                "214DFF47469D462E1D9A03232C2582C7E44DE026A287F98529CC74DE9BD69641"
            ),
            cosignature.parent_hash,
        )
    ...
    def test_can_sign_and_verify(self):
        ...
        is_verified = SymbolFacade.Verifier(key_pair.public_key).verify(
            message, signature
        )
    ...
    def test_can_create_via_repository(self):
        ...
        self.assertEqual(
            SymbolFacade.Address("TASYMBOLLK6FSL7GSEMQEAWN7VW55ZSZU2Q2Q5Y").bytes,
    ...
    def test_can_create_public_account_from_public_key(self):
        ...
        # Assert:
        self.assertEqual(
            facade.Address("TABDOFVM2QYIMVNQII6UJWU7Y66GZI4LQTMN4PI"), account.address
        )
    ...
    def _get_cosign_transaction_test_descriptor_for_account_wrappers():
        return CosignTransactionTestDescriptor(
            lambda facade, private_key, transaction: facade.create_account(
                private_key
            ).sign_transaction(transaction),
            lambda facade, private_key, transaction, detached: facade.create_account(
                private_key
            ).cosign_transaction(transaction, detached),
        )

    def test_can_cosign_transaction_with_account_wrappers(self):
        self._run_test_can_cosign_transaction(
            self._get_cosign_transaction_test_descriptor_for_account_wrappers()
        )

    def test_can_cosign_transaction_detached_with_account_wrappers(self):
        self._run_test_can_cosign_transaction_detached(
            self._get_cosign_transaction_test_descriptor_for_account_wrappers()
        )

    @staticmethod
    def _get_cosign_transaction_hash_test_descriptor_for_account_wrappers():
        def _cosign_transaction_hash_for_account_wrappers(
            facade, private_key, transaction, detached
        ):
            transaction_hash = facade.hash_transaction(transaction)
            return facade.create_account(private_key).cosign_transaction_hash(
                transaction_hash, detached
            )
    ...
    def test_can_cosign_transaction_hash_with_account_wrappers(self):
        self._run_test_can_cosign_transaction(
            self._get_cosign_transaction_hash_test_descriptor_for_account_wrappers()
        )
    ...
    def _assert_can_hash_transaction(self, transaction_factory, expected_hash):
        ...
        signature = facade.sign_transaction(
            SymbolFacade.KeyPair(private_key), transaction
        )
        facade.transaction_factory.attach_signature(transaction, signature)
    ...
    def test_can_sign_transaction(self):
        self._assert_can_sign_transaction(
            self._create_real_transfer,
            Signature(
                "".join(
                    [
    ...
    def test_can_verify_signed_transaction(self):
        self._assert_can_verify_transaction(
            self._create_real_transfer, self._sign_transaction
        )

    def test_can_verify_signed_aggregate_transaction(self):
        self._assert_can_verify_transaction(
            self._create_real_aggregate, self._sign_transaction
        )

    def test_can_verify_signed_transaction_signing_payload(self):
        self._assert_can_verify_transaction(
            self._create_real_transfer, self._sign_transaction_signing_payload
        )

    def test_can_verify_signed_aggregate_transaction_signing_payload(self):
        self._assert_can_verify_transaction(
            self._create_real_aggregate, self._sign_transaction_signing_payload
        )
    ...
    def _get_cosign_transaction_test_descriptor_for_facade():
        return CosignTransactionTestDescriptor(
            lambda facade, private_key, transaction: facade.sign_transaction(
                facade.KeyPair(private_key), transaction
            ),
            lambda facade,
            private_key,
            transaction,
            detached: facade.cosign_transaction(
                facade.KeyPair(private_key), transaction, detached
            ),
        )

    def test_can_cosign_transaction(self):
        self._run_test_can_cosign_transaction(
            self._get_cosign_transaction_test_descriptor_for_facade()
        )
    ...
    def _get_cosign_transaction_hash_test_descriptor_for_facade():
        def _cosign_transaction_hash_for_facade(
            facade, private_key, transaction, detached
        ):
            transaction_hash = facade.hash_transaction(transaction)
            return facade.cosign_transaction_hash(
                facade.KeyPair(private_key), transaction_hash, detached
            )
    ...
    def test_can_cosign_transaction_hash(self):
        self._run_test_can_cosign_transaction(
            self._get_cosign_transaction_hash_test_descriptor_for_facade()
        )
    ...
    def test_can_hash_embedded_transactions_multiple(self):
        ...
        # Assert:
        self.assertEqual(
            Hash256("5C78999F21EA75B880100E1B4C76166B9C320869F67C00D28F9F8F754D7831C9"),
            hash_value,
        )
    ...
    def _assert_bip32_child_public_keys(self, passphrase, expected_child_public_keys):
        ...
        # Act:
        root_node = Bip32(SymbolFacade.BIP32_CURVE_NAME).from_mnemonic(
            mnemonic_seed, passphrase
        )
    ...
    def test_can_use_bip32_derivation_without_passphrase(self):
        self._assert_bip32_child_public_keys(
            ...
            [
                PublicKey(
                    "E9CFE9F59CB4393E61B2F42769D9084A644B16883C32C2823E7DF9A3AF83C121"
                ),
                PublicKey(
                    "0DE8C3235271E4C9ACF5482F7DFEC1E5C4B20FFC71548703EACF593153F116F9"
                ),
                PublicKey(
                    "259866A68A00C325713342232056333D60710E223FC920566B3248B266E899D5"
                ),
            ],
        )
    ...
    def test_can_use_bip32_derivation_with_passphrase(self):
        self._assert_bip32_child_public_keys(
            ...
            [
                PublicKey(
                    "47F4D39D36D11C07735D7BE99220696AAEE7B3EE161D61422220DFE3FF120B15"
                ),
                PublicKey(
                    "4BA67E87E8C14F3EB82B3677EA959B56A9D7355705019CED1FCF6C76104E628C"
                ),
                PublicKey(
                    "8115D75C13C2D25E7FA3009D03D63F1F32601CDCCA9244D5FDAC74BCF3E892E3"
                ),
            ],
        )
