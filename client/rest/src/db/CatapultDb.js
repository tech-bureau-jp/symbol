...
import { Network, models } from "@tech-bureau/symbol-sdk/symbol";

const { ObjectId } = MongoDb;

const isAggregateType = (document) =>
  models.TransactionType.AGGREGATE_COMPLETE.value ===
...
const createSanitizer = () => ({
  ...
  deleteId: (dbObject) => {
    if (dbObject) delete dbObject._id;
...
const mapToPromise = (dbObject) =>
  Promise.resolve(null === dbObject ? undefined : dbObject);
...
export default class CatapultDb {
  ...
  constructor(options) {
    ...
    if (!this.networkId) throw Error("network id is required");
  ...
  connect(url, dbName, connectionPoolSize, timeout) {
    return connector
      .connectToDatabase(url, dbName, connectionPoolSize || 10, timeout)
      .then((client) => {
        this.client = client;
  ...
  queryDocument(collectionName, conditions, projection) {
    ...
    return collection.findOne(conditions, { projection }).then(mapToPromise);
  }

  queryDocuments(collectionName, conditions) {
    const collection = this.database.collection(collectionName);
    return collection.find(conditions).toArray().then(this.sanitizer.deleteIds);
  }
   ...
   */
  storageInfo() {
    const blockCountPromise = this.database
      .collection("blocks")
      .estimatedDocumentCount();
    const transactionCountPromise = this.database
      .collection("transactions")
      .estimatedDocumentCount();
    const accountCountPromise = this.database
      .collection("accounts")
      .estimatedDocumentCount();
  ...
  chainStatisticCurrent() {
    return this.queryDocument("chainStatistic", {}, { _id: 0 }).then(
      (chainStatistic) => chainStatistic.current
    );
  }

  latestFinalizedBlock() {
    return this.database
      .collection("finalizedBlocks")
  ...
  blocks(
    ...
    const removeFields = [
      "meta.transactionMerkleTree",
      "meta.statementMerkleTree",
    ];

    const sortConditions = {
      [sortingOptions[options.sortField]]: options.sortDirection,
    };

    return this.queryPagedDocuments(
      conditions,
      removeFields,
      sortConditions,
      "blocks",
    ...
    return this.queryDocumentsAndCopyIds(
      ...
      { "block.height": { $in: heights } },
      {
        projection: projection || {
          "meta.transactionMerkleTree": 0,
          "meta.statementMerkleTree": 0,
        },
      }
    );
  }
  ...
  blockWithMerkleTreeAtHeight(height, merkleTreeName) {
    const blockMerkleTreeNames = [
      "transactionMerkleTree",
      "statementMerkleTree",
    ];
    const excludedMerkleTrees = {};
    blockMerkleTreeNames
      .filter((merkleTree) => merkleTree !== merkleTreeName)
      .forEach((merkleTree) => {
        excludedMerkleTrees[`meta.${merkleTree}`] = 0;
      });
    return this.queryDocument(
      "blocks",
      { "block.height": convertToLong(height) },
  ...
  latestBlocksFeeMultiplier(numBlocks) {
    if (0 === numBlocks) return Promise.resolve([]);
    ...
    return this.database
      ...
      .toArray()
      .then((blocks) =>
        Promise.resolve(blocks.map((block) => block.block.feeMultiplier))
      );
  }

  queryDependentDocuments(collectionName, aggregateIds) {
    if (0 === aggregateIds.length) return Promise.resolve([]);

    return this.queryDocumentsAndCopyIds(collectionName, {
      "meta.aggregateId": { $in: aggregateIds },
    });
  }
  ...
  async transactions(group, filters, options) {
    ...
    const buildAccountConditions = async () => {
      ...
      if (undefined !== filters.address) {
        if ("partial" === group) {
          const multisigEntries = await new MultisigDb(
            this
          ).multisigsByAddresses([filters.address]);

          if (
            multisigEntries.length &&
            multisigEntries[0].multisig.multisigAddresses.length
          ) {
            const buffers = multisigEntries[0].multisig.multisigAddresses.map(
      ...
      if (undefined !== filters.signerPublicKey)
        accountConditions["transaction.signerPublicKey"] = Buffer.from(
          filters.signerPublicKey
        );
      if (undefined !== filters.recipientAddress)
        accountConditions["transaction.recipientAddress"] = Buffer.from(
    ...
    const buildConditions = async () => {
      ...
      if (undefined !== filters.transferMosaicId)
        conditions["transaction.mosaics.id"] = convertToLong(
          filters.transferMosaicId
        );

      if (
        undefined !== filters.fromTransferAmount ||
        undefined !== filters.toTransferAmount
      ) {
        const amountPath = "transaction.mosaics.amount";
        conditions[amountPath] = {};

        if (undefined !== filters.fromTransferAmount)
          conditions[amountPath].$gte = convertToLong(
            filters.fromTransferAmount
          );
  ...
  transactionsByIdsImpl(collectionName, conditions) {
    return this.queryDocumentsAndCopyIds(collectionName, conditions, {
      projection: { "meta.addresses": 0 },
    })
      .then((list) => this.addBlockMetaToTransactionList(list))
      .then((documents) =>
        Promise.all(
          documents.map((document) => {
            if (!document || !isAggregateType(document)) return document;

            return this.queryDependentDocuments(collectionName, [
              document.id,
            ]).then((dependentDocuments) => {
              dependentDocuments.forEach((dependentDocument) => {
                if (!document.transaction.transactions)
                  document.transaction.transactions = [];
                if (
                  document.meta.timestamp !== undefined &&
                  document.meta.feeMultiplier !== undefined
                ) {
                  dependentDocument.meta.timestamp = document.meta.timestamp;
                  dependentDocument.meta.feeMultiplier =
                    document.meta.feeMultiplier;
                }
  ...
  addBlockMetaToTransactionList(list) {
    return this.addBlockMetaToEntityList(
      list,
  ...
  async addBlockMetaToEntityList(list, fields, getHeight) {
    ...
    return list.map((item) => {
      ...
      if (!isValidHeight(height)) return item;
      const block = blocks.find((blockInfo) =>
        blockInfo.block.height.equals(height)
      );
      if (!block)
  ...
  transactionsByIds(group, ids) {
    return this.transactionsByIdsImpl(TransactionGroup[group], {
  ...
  transactionsByHashes(group, hashes) {
    return this.transactionsByIdsImpl(TransactionGroup[group], {
      "meta.hash": { $in: hashes.map((hash) => Buffer.from(hash)) },
    });
  }
  ...
  findNamesByIds(ids, transactionType, fieldNames) {
    ...
    const grouping = {
      $group: {
        ...
        [fieldNames.parentId]: {
          $first: `$transaction.${fieldNames.parentId}`,
        },
      },
    };
    ...
    return collection
      .aggregate([conditions, grouping], { promoteLongs: false })
      .sort({ _id: -1 })
      .toArray()
  ...
  accounts(address, mosaicId, options) {
    ...
    if ("balance" === options.sortField) {
      ...
      queryPromise = this.database
        ...
        .then((accounts) => {
          ...
          return this.queryPagedDocuments(newConditions, [], {}, "accounts", {
            ...
            fullAccountsPage.data.sort(
              (account1, account2) =>
                accountIds.findIndex((accountId) =>
                  accountId.equals(account1.id)
                ) -
                accountIds.findIndex((accountId) =>
                  accountId.equals(account2.id)
                )
            );
      ...
      queryPromise = this.queryPagedDocuments(
        conditions,
        [],
        sortConditions,
        "accounts",
  ...
  transactionsByHashesFailed(hashes) {
    ...
    return this.queryDocuments("transactionStatuses", {
      "status.hash": { $in: buffers },
    });
  }
  ...
  lookupPublicKeys(accountAddresses) {
    const conditions = {
      "account.address": {
        $in: accountAddresses.map((accountAddress) =>
