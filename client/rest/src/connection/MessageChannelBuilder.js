...
const createBlockDescriptor = (marker, handler) => ({
  filter: (topicParam) => {
    if (topicParam) throw new Error("unexpected param to block subscription");
...
const createPolicyBasedAddressFilter =
  (markerByte, emptyAddressHandler, networkIdentifier) => (topicParam) => {
    if (!topicParam) return emptyAddressHandler(markerByte);
    ...
    try {
      ...
      return Buffer.concat([
        Buffer.of(markerByte),
        Buffer.from(addressByteArray),
      ]);
    } catch (err) {
...
export default class MessageChannelBuilder {
  ...
  constructor(config, networkIdentifier) {
    ...
    const emptyAddressHandler =
      config && config.allowOptionalAddress
    ...
    this.createAddressFilter = (markerChar) =>
      createPolicyBasedAddressFilter(
        markerChar.charCodeAt(0),
        emptyAddressHandler,
        networkIdentifier
      );
  ...
  add(name, markerChar, handler) {
    ...
    this.descriptors[name] = {
      filter: this.createAddressFilter(markerChar),
      handler,
    };
