  ...
  register: (server, db, services) => {
    ...
    server.get("/namespaces", (req, res, next) => {
      ...
      const ownerAddress = params.ownerAddress
        ? routeUtils.parseArgument(params, "ownerAddress", "address")
        : undefined;
      const registrationType = params.registrationType
        ? routeUtils.parseArgument(params, "registrationType", "uint")
        : undefined;
      const level0 = params.level0
        ? routeUtils.parseArgument(
            req.params,
            "level0",
            routeUtils.namedParserMap.uint64hex
          )
        : undefined;
      const aliasType = params.aliasType
        ? routeUtils.parseArgument(params, "aliasType", "uint")
        : undefined;

      const options = routeUtils.parsePaginationArguments(
        req.params,
        services.config.pageSize,
        { id: "objectId" }
      );
    ...
    server.get("/namespaces/:namespaceId", (req, res, next) => {
      const namespaceId = routeUtils.parseArgument(
        req.params,
        "namespaceId",
        routeUtils.namedParserMap.uint64hex
      );
    ...
    const collectNames = (namespaceNameTuples, namespaceIds) => {
      ...
      return db.catapultDb
        .findNamesByIds(namespaceIds, type, {
          id: "id",
          name: "name",
          parentId: "parentId",
        })
        .then((nameTuples) => {
          nameTuples.forEach((nameTuple) => {
            ...
            if (null === nameTuple.parentId) delete nameTuple.parentId;
    ...
    server.post("/namespaces/names", (req, res, next) => {
      const namespaceIds = routeUtils.parseArgumentAsArray(
        req.params,
        "namespaceIds",
        routeUtils.namedParserMap.uint64hex
      );
      const nameTuplesFuture = new Promise((resolve) => {
        ...
        const chain = (nextIds) => {
          if (0 === nextIds.length) resolve(namespaceNameTuples);
          else collectNames(namespaceNameTuples, nextIds).then(chain);
        };

        collectNames(namespaceNameTuples, namespaceIds).then(chain);
      });

      return nameTuplesFuture.then(
        routeUtils
          .createSender("namespaceNameTuple")
          .sendArray("namespaceIds", res, next)
      );
    });

    server.post(
      "/namespaces/mosaic/names",
      namespaceUtils.aliasNamesRoutesProcessor(
        db,
    ...
    server.post(
      ...
      namespaceUtils.aliasNamesRoutesProcessor(
        ...
        (req) =>
          routeUtils.parseArgumentAsArray(req.params, "addresses", "address"),
        (namespace, id) =>
          Buffer.from(namespace.namespace.alias.address.value()).equals(
            Buffer.from(new Binary(Buffer.from(id)).value())
          ),
        "address",
        "accountNames"
      )
    ...
    server.get("/namespaces/:namespaceId/merkle", (req, res, next) => {
      ...
      return merkleUtils
        .requestTree(services, state, utils.intToBytes(namespaceId, 8))
        .then((response) => {
          res.send(response);
          next();
        });
