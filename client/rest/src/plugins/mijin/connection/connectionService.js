...
export default (config, logger = () => {}) => {
  ...
  const openAuthorizedConnection = (isPersistent) => {
    ...
    const connectionPromise = new Promise((resolve, reject) => {
      ...
      serverSocket
        .once("secureConnect", () => {
          if (serverSocket.authorized || serverSocket.authorizationError) {
        ...
        .on("close", () => {
          if (isPersistent) delete aliveConnections[node];

          reject(errors.createServiceUnavailableError("connection failed"));
        });
    });

    if (isPersistent) authorizingConnectionPromises[node] = connectionPromise;
  ...
  return {
    ...
    lease: () => {
      ...
      if (authorizingPromise) return authorizingPromise;
