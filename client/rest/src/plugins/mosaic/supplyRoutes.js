...
export default {
  register: (server, db, services) => {
    const convertToFractionalWholeUnits = (value, divisibility) =>
      (Number(value) / 10 ** divisibility).toFixed(divisibility);
    ...
    const getUncirculatingAccountIds = (propertiesObject) => {
      const publicKeys = [
        propertiesObject.network.nemesisSignerPublicKey,
      ].concat(services.config.uncirculatingAccountPublicKeys);
      return publicKeys.map((publicKey) => ({
        [AccountType.publicKey]: utils.hexToUint8(publicKey),
      }));
    };
    ...
    const lookupMosaicAmount = (mosaics, currencyMosaicId) => {
      ...
      return undefined === matchingMosaic
        ? 0
        : matchingMosaic.amount.toNumber();
    };

    server.get("/network/currency/supply/circulating", (req, res, next) =>
      readAndParseNetworkPropertiesFile().then(async (propertiesObject) => {
        const currencyMosaicId = propertyValueToMosaicId(
          propertiesObject.chain.currencyMosaicId
        );
        const currencyMosaicProperties = await getMosaicProperties(
          currencyMosaicId
        );

        const accounts = await db.catapultDb.accountsByIds(
          getUncirculatingAccountIds(propertiesObject)
        );
        const burnedSupply = accounts.reduce(
          (sum, account) =>
            sum + lookupMosaicAmount(account.account.mosaics, currencyMosaicId),
          0
        );
    ...
    server.get("/network/currency/supply/total", (req, res, next) =>
      readAndParseNetworkPropertiesFile().then(async (propertiesObject) => {
        const currencyMosaicId = propertyValueToMosaicId(
          propertiesObject.chain.currencyMosaicId
        );
    ...
    server.get("/network/currency/supply/max", (req, res, next) =>
      readAndParseNetworkPropertiesFile().then(async (propertiesObject) => {
        const currencyMosaicId = propertyValueToMosaicId(
          propertiesObject.chain.currencyMosaicId
        );
        ...
        const maxSupply = parseInt(
          propertiesObject.chain.maxMosaicAtomicUnits.replace(/'/g, ""),
          10
        );
        sendPlainText(
          res,
