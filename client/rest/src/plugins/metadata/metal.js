...
export const metal = {
  ...
  extractCompositeHashFromMetalId(metalId) {
    ...
    const isValidMetalId =
      METAL_ID_HEADER_SIGNATURE[0] === decodedBs58HashBytes[0] &&
      METAL_ID_HEADER_SIGNATURE[1] === decodedBs58HashBytes[1] &&
      METAL_ID_LENGTH === decodedBs58HashBytes.length;

    if (!isValidMetalId) throw Error(`'${metalId}' is not a valid metal id`);

    return decodedBs58HashBytes.subarray(METAL_ID_HEADER_SIGNATURE.length);
  },
  ...
  extractChunk(buffer) {
    ...
    const scopedMetadataKey = utils.bytesToBigInt(
      new Uint8Array(buffer.subarray(4, 12)).reverse(),
      8
    );
  ...
  decode(firstKey, chunks) {
    ...
    do {
      ...
      if (!chunk) throw new Error(`the chunk ${scopedMetadataKey} is missing`);
      ...
      if (!isValidMetadataKey)
        throw new Error(
          `the chunk ${scopedMetadataKey} is broken (calculated=${metadataKey})`
        );

      const chunkData = this.extractChunk(chunk.value);
      ({ magic, scopedMetadataKey } = chunkData);
      const { chunkPayload, chunkText } =
        this.splitChunkPayloadAndText(chunkData);
      if (chunkPayload.length) decodedPayloads.push(chunkPayload);

      if (chunkText?.length) decodedTexts.push(chunkText);
    } while (0x80 !== magic);
    return {
      payload: Buffer.concat(decodedPayloads),
      text: decodedTexts.length
        ? Buffer.concat(decodedTexts).toString("utf-8")
        : undefined,
    };
  },
};
...
export class MetalSeal {
   ...
   */
  stringify() {
    const result = [this.schema, this.length];

    if (this.mimeType) result.push(this.mimeType);
    else if (this.name || this.comment) result.push(null);

    if (this.name) result.push(this.name);
    else if (this.comment) result.push(null);

    if (this.comment) result.push(this.comment);
