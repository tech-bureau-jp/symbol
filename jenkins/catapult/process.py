...
class ProcessManager:
    ...
    def dispatch_subprocess(
        self, command_line, show_output=True, handle_error=True, redirect_filename=None
    ):
            ...
            if handle_error:
                if 0 != process.returncode:
                    ...
                    raise SubprocessError(
                        f"{formatted_command_line} exited with {process.returncode}"
                    )
            else:
        ...
        with Popen(command_line, stdout=PIPE, stderr=STDOUT) as process:
            ...
            for line_bin in iter(process.stdout.readline, b""):
                ...
                if is_filtered_output:
                    ...
                    if "suite" == verbosity and any(
                        line.startswith(prefix)
                        for prefix in ("[ RUN      ]", "[       OK ]")
                    ):
                        continue
