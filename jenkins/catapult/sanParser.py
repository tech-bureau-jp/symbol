...
class AsanParser(BasicParser):
    ...
    def __init__(self):
        super().__init__(
            self.ParserState.START_MARKER, "SUMMARY", self.ParserState.START_MARKER
        )
    ...
    def create_asan_error():
        return SanError(
            r"==ERROR: ([^:]*)(: )(.*)", lambda match: ("sanitizer", match.group(1))
        )
    ...
    def create_ubsan_error():
        return SanError(
            r"^([^:]*):(\d+):\d+: (.*) 0x.*",
            lambda match: ("sanitizer", match.group(1)),
        )
...
class TsanParser(BasicParser):
    ...
    def __init__(self):
        super().__init__(
            self.ParserState.START_MARKER, "SUMMARY", self.ParserState.END_MARKER
        )

        self.handlers = {
            self.ParserState.START_MARKER: self.parse_start_header,
            self.ParserState.WARNING: self.create_checker(
                "WARNING: ", self.ParserState.DETAILS
            ),
            self.ParserState.DETAILS: self.parse_details,
            self.ParserState.END_MARKER: self.create_checker(
                "==================", self.ParserState.START_MARKER
            ),
        }
...
def main():
    ...
    parser.add_argument(
        "--mode", help="parsing mode", choices=("asan", "tsan"), required=True
    )
