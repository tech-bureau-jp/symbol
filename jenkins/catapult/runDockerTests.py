...
def main():
    ...
    parser.add_argument(
        "--mode", help="test mode", choices=("bench", "test", "lint"), required=True
    )
    ...
    prepare_replacements = {
        ...
        "linter_path": str(Path(args.linter_path).resolve().absolute())
        if is_lint_enabled(args.mode)
        else "",
    }
    prepare_docker_compose_file(
        compose_template_filepath, prepare_replacements, sys.stdout
    )

    if not args.dry_run:
        with open("docker-compose.yaml", "wt", encoding="utf8") as outfile:
            prepare_docker_compose_file(
                compose_template_filepath, prepare_replacements, outfile
            )
