...
class EnvironmentManager:
    ...
    def system_bin_path(self):
        ...
        for descriptor in [
            ...
            ("fedora", "/usr/lib64"),
            ("ubuntu arm64", "/usr/lib/aarch64-linux-gnu"),
        ]:
            if Path(descriptor[1]).exists():
                self._print_command(
                    "system_bin_path", ["detected", descriptor[1], "for", descriptor[0]]
                )
                return descriptor[1]
    ...
    def rmtree(self, path):
        ...
        kwargs = (
            {"onexc": rm_onexc_handler}
            if sys.version_info >= (3, 12)
            else {"onerror": rm_onerror_handler}
        )
    ...
    def root_directory(directory_name):
        root_directory_prefix = (
            "c:\\" if EnvironmentManager.is_windows_platform() else "/"
        )
