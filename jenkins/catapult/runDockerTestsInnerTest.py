...
def prepare_tests(environment_manager, source_path, output_path):
    environment_manager.copy_tree_with_symlinks(
        f"{source_path}/seed", f"{output_path}/seed"
    )
    environment_manager.copy_tree_with_symlinks(
        f"{source_path}/resources", f"{output_path}/resources"
    )

    environment_manager.mkdirs(f"{output_path}/tests/int/stress")
    environment_manager.copy_tree_with_symlinks(
        f"{source_path}/tests/int/stress/resources",
        f"{output_path}/tests/int/stress/resources",
    )


def process_sanitizer_logs(environment_manager, output_directory, san_descriptor):
    counter = 1
    for logfile_name in environment_manager.find_glob(
        ".", san_descriptor["input_pattern"]
    ):
        log_name = (
            output_directory
            / f"{san_descriptor['output_filename_prefix']}.{counter}.xml"
        )
...
def handle_core_file(
    process_manager, core_path, test_exe_filepath, base_output_filepath
):
...
def main():
    ...
    parser.add_argument(
        "--out-dir", help="directory in which to store test output files", required=True
    )
    parser.add_argument(
        "--verbosity",
    ...
    if EnvironmentManager.is_windows_platform():
        ...
        environment_manager.set_env_var(
            "PATH", f"{path};{output_path}/lib;{output_path}/deps"
        )
    else:
        environment_manager.set_env_var(
            "LD_LIBRARY_PATH", f"{output_path}/lib:{output_path}/deps"
        )
    ...
    failed_test_suites = []
    test_filter = (
        "test*" if not EnvironmentManager.is_windows_platform() else "test*.exe"
    )
    for test_exe_filepath in environment_manager.find_glob(args.exe_path, test_filter):
        ...
        process_sanitizer_logs_all(
            environment_manager, logs_path, test_exe_filepath.name
        )
