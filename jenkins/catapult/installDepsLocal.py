...
class Downloader:
    ...
    def download_boost_unix(self):
        ...
        tar_source_path = (
            f"https://archives.boost.io/release/{version}/source/{tar_filename}"
        )

        self.process_manager.dispatch_subprocess(
            ["curl", "-o", tar_filename, "-SL", tar_source_path]
        )
    ...
    def download_boost_windows(self):
        ...
        zip_source_path = (
            f"https://archives.boost.io/release/{version}/source/{zip_filename}"
        )
        ...
        self.process_manager.dispatch_subprocess(
            ["powershell", "-Command", "7z", "x", zip_filename]
        )
        self.process_manager.dispatch_subprocess(["powershell", "-Command", "dir"])
        self.process_manager.dispatch_subprocess(
            ["powershell", "-Command", "Move-Item", rf"{archive_name}", "boost"]
        )
...
class Builder:
    def __init__(
        self, target_directory, versions, process_manager, environment_manager
    ):
    ...
    def build_boost(self):
        self.environment_manager.chdir(
            self.target_directory / SOURCE_DIR_NAME / "boost"
        )
        ...
        self.process_manager.dispatch_subprocess(
            [b2_filepath]
            + b2_options
            + ["-j", str(NUM_BUILD_CORES), "stage", "release"]
        )
    ...
    def build_git_dependency(self, organization, project):
        self.environment_manager.chdir(
            self.target_directory / SOURCE_DIR_NAME / project
        )
        ...
        cmake_options = [
            ...
            f"-DCMAKE_INSTALL_PREFIX={self.target_directory / organization}",
        ]
        ...
        if EnvironmentManager.is_windows_platform() and "mongo-cxx-driver" == project:
            ...
            cmake_options += [
                ...
                f"-DCMAKE_PREFIX_PATH={self.target_directory / organization}",
            ]
    ...
    def build_openssl_windows(self):
        openssl_destinations = [
            f"--{key}={self.target_directory / 'openssl'}"
            for key in ("prefix", "openssldir")
        ]
        self.process_manager.dispatch_subprocess(
            ["perl", "./Configure", "VC-WIN64A"] + openssl_destinations
        )
    ...
    def build_openssl_unix(self):
        ...
        openssl_destinations = [
            f"--{key}={self.target_directory / 'openssl'}"
            for key in ("prefix", "openssldir", "libdir")
        ]
        self.process_manager.dispatch_subprocess(
            ["perl", "./Configure", compiler] + openssl_destinations
        )
...
def main():
    ...
    parser.add_argument(
        "--download", help="download all dependencies", action="store_true"
    )
    ...
    parser.add_argument(
        "--use-clang", help="uses clang compiler instead of gcc", action="store_true"
    )
    parser.add_argument(
        "--dry-run",
    ...
    parser.add_argument(
        "--force", help="purges any existing files", action="store_true"
    )
    ...
    if args.build:
        ...
        builder = Builder(
            target_directory, versions, process_manager, environment_manager
        )
        if args.use_clang:
            builder.use_clang()
