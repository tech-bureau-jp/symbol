...
def find_files_with_extension(environment_manager, directory, extension):
    return list(
        environment_manager.find_glob(directory, f"*{extension}", recursive=True)
    )
...
def run_cpp_linters(process_manager, dest_dir):
    cpp_lint_args = [
        "python3",
        str(LINTER_DIR.joinpath("cpp/checkProjectStructure.py").resolve()),
    ]
...
class LinterRunner:
    ...
    def run(self, args):
        linter_result = self.process_manager.dispatch_subprocess(
            args, redirect_filename=self.output_filepath
        )
...
def run_python_linters(linter_runner, python_files, script_path):
    ...
    linter_runner.run(
        [
            "pylint",
            "--rcfile",
            str(LINTER_DIR.joinpath("python/.pylintrc").resolve()),
            "--output-format",
            "parseable",
        ]
        + python_files
    ...
    linter_runner.fixup(
        lambda line: line
        if not pylint_warning_pattern.match(line)
        else f"{script_path}/{line}"
    )
    ...
    linter_runner.run(
        [
            "pycodestyle",
            "--config",
            str(LINTER_DIR.joinpath("python/.pycodestyle").resolve()),
        ]
        + python_files
    )
    ...
    linter_runner.fixup(
        lambda line: isort_warning_pattern.sub(
...
def main():
    ...
    parser.add_argument(
        "--out-dir", help="directory in which to store lint output files", required=True
    )
    parser.add_argument(
        "--dry-run",
    ...
    if not EnvironmentManager.is_windows_platform():
        run_shell_linters(
            linter_runner,
            find_files_with_extension(environment_manager, source_dir, ".sh"),
        )

    run_python_linters(
        linter_runner,
        find_files_with_extension(environment_manager, source_dir, ".py"),
        str(script_path),
    )
