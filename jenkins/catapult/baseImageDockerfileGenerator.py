...
BOOST_DISABLED_LIBS = map(
    lambda library_name: f"--without-{library_name}",
    [
        "context",
        "contract",
        "coroutine",
...
class OptionsManager:
    ...
    def b2(self):  # pylint: disable=invalid-name
        ...
        if self.is_clang:
            ...
            options += [
                format_multivalue_options("linkflags", [f"-stdlib={self.stl.lib}"])
            ]
    ...
    def _stl_flags(self):
        return (
            None
            if not self.stl
            else [f"-std={self.stl.version}", f"-stdlib={self.stl.lib}"]
        )
    ...
    def _cmake(self, descriptor):
        ...
        if self.is_clang:
            descriptor.options += [
                format_multivalue_options("-DCMAKE_CXX_COMPILER", [self.compiler.cpp])
            ]
        ...
        if descriptor.sanitizer:
            ...
            descriptor.options += [
                format_multivalue_options("-DCMAKE_C_FLAGS", [sanitize_flag])
            ]
        ...
        if descriptor.cxxflags:
            descriptor.options += [
                format_multivalue_options("-DCMAKE_CXX_FLAGS", descriptor.cxxflags)
            ]

        if descriptor.linkflags:
            descriptor.options += [
                format_multivalue_options(
                    "-DCMAKE_SHARED_LINKER_FLAGS", descriptor.linkflags
                ),
                format_multivalue_options(
                    "-DCMAKE_EXE_LINKER_FLAGS", descriptor.linkflags
                ),
            ]
...
class WindowsSystem:
    @staticmethod
    def add_base_os_packages():
        scoop_packages = ["ccache", "git", "python", "cmake", "7zip"]
        print_powershell_lines(
            ["scoop update", "scoop install {SCOOP_PACKAGES}"],
    ...
    def add_test_packages():
        print_powershell_lines(
            ["scoop update", "python3 -m pip install -U pycodestyle pylint pyyaml"]
        )
...
class LinuxSystemGenerator:
    ...
    def generate_phase_boost(self):
        ...
        print_line(
            [
                ...
                "cd ..",
                "rm -rf {BOOST_ARCHIVE} {BOOST_ARCHIVE}.tar.gz",
            ],
            **print_args,
        )
    ...
    def generate_phase_test(self):
        ...
        self.add_openssl(
            self.options,
            self.options.openssl_configure() if self.options.sanitizers else [],
        )
...
class WindowsSystemGenerator:
    ...
    def add_git_dependency(self, organization, project, package_options, revision=1):
        ...
        generator = (
            ...
            if 16 == self.options.compiler.version
    ...
    def generate_phase_test(self):
        ...
        print_lines(['RUN echo "docker image build $BUILD_NUMBER"'])
...
def main():
    parser = argparse.ArgumentParser(
        description="catapult base image dockerfile generator"
    )
    parser.add_argument(
        "--layer",
        help="name of docker layer to generate",
        choices=LAYER_TO_IMAGE_TAG_MAP.keys(),
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--name-only", help="true to output layer name", action="store_true"
    )
    parser.add_argument(
        "--base-name-only", help="true to output base name", action="store_true"
    )
    parser.add_argument(
        "--ignore-architecture",
        help="ignore architecture for image name",
        action="store_true",
    )
    ...
    if args.ignore_architecture:
        print(
            "error: ignore architecture can only be used with name-only or base-name-only"
        )
        sys.exit(1)

    system_generator_type = (
        WindowsSystemGenerator
        if "windows" == args.operating_system
        else LinuxSystemGenerator
    )
    dockerfile_generator = system_generator_type(
        SYSTEMS[args.operating_system], options_manager
    )
    {
