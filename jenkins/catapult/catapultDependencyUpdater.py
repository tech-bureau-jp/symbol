...
class CatapultDependencyUpdater:
    ...
    def _setup_conan_environment():
        _, error_code = dispatch_subprocess(
            ["conan", "profile", "show", "--profile", "default"], handle_error=False
        )
        if 0 != error_code:
    ...
    def _get_conan_latest_version(self, name):
        query = (
            f"{name}/*"
            if not self._is_remote_nemtech(name)
            else f"{name}/*@{NEMTECH_REMOTE_NAME}/stable"
        )
        remote_name = (
            NEMTECH_REMOTE_NAME if self._is_remote_nemtech(name) else "conancenter"
        )
        output, _ = dispatch_subprocess(
            ["conan", "search", query, "--remote", remote_name]
        )
        version_list = [
        ...
        filtered_versions = [
            version for version in version_list if version[0].isdigit()
        ]
    ...
    def get_available_update(self, dependencies):
        ...
        for dependency in dependencies:
            ...
            current_version = re.sub(
                r"[^\d\.]", "", self.versions[DEPENDENCY_NAMES_MAP[dependency][0]]
            )
            print(
                f"checking dependency {dependency} {current_version} -> {latest_version}"
            )
            if Version(latest_version) > Version(current_version):
                print(f"{dependency} {current_version} -> {latest_version}")
                dependencies_with_updates.append(
    ...
    def update_dependencies(self, dependencies_to_update):
        def update_version_in_file(
            name, filepath, old_version, new_version, get_current_version
        ):
            ...
            dispatch_subprocess(
                [
                    "sed",
                    "-i",
                    f"s|{current_name_version}|{new_name_version}|g",
                    str(filepath),
                ]
            )
        ...
        for dependency_name, current_version, latest_version in dependencies_to_update:
            ...
            print(
                f"updating dependency {dependency_name} from {current_version} -> {latest_version}"
            )
...
def main():
    ...
    parser.add_argument(
        "--dependencies",
        choices=DEPENDENCIES_NAME,
        nargs="*",
        help="dependencies to update",
        default=DEPENDENCIES_NAME,
    )
    parser.add_argument(
        "--versions-file", help="path to the versions file", required=True
    )
    parser.add_argument(
        "--source-path", help="path to the catapult source", required=True
    )
    ...
    update_message = "\n".join(
        [
            f"{dependency[0]} {dependency[1]} -> {dependency[2]}"
    ...
    if args.commit:
        ...
        dispatch_subprocess(
            [
                ...
                "commit",
                "-m",
                f"[client/catapult] fix: update catapult dependency\n\n{update_message}",
            ]
        )
