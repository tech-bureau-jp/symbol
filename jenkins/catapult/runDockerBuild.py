...
class OptionsManager(BasicBuildManager):
    ...
    def docker_run_settings(self):
        ...
        if not self.is_msvc:
            settings.extend([("CC", self.compiler.c), ("CXX", self.compiler.cpp)])
...
def get_volume_mappings(ccache_path, conan_path, prepare_replacements):
    mappings = [
        (
            prepare_replacements["source_path"],
            EnvironmentManager.root_directory("catapult-src"),
        ),
        ...
        (
            prepare_replacements["script_path"],
            EnvironmentManager.root_directory("scripts"),
        ),
    ]
...
def create_docker_run_command(options, prepare_replacements):
    ...
    volume_mappings = get_volume_mappings(
        options.ccache_path, options.conan_path, prepare_replacements
    )
...
def cleanup_directories(
    environment_manager, ccache_root_directory, conan_root_directory
):
...
def prepare_docker_image(process_manager, container_id, prepare_replacements):
    ...
    disposition_to_repository_map = {
        "tests": "catapult-server-test",
        "private": "catapult-server-private",
        "mijin": "catapult-server",
        "mijintest": "catapult-server-test",
        "public": "catapult-server",
    }
...
def main():
    ...
    parser.add_argument(
        ...
        help="docker destination image label",
        required=True,
    )
    parser.add_argument(
        "--dry-run",
    ...
    parser.add_argument(
        ...
        help="only output the base image names",
        action="store_true",
    )
    ...
    docker_run = create_docker_run_command(
        options,
        {
    ...
    for folder_name in ["scripts", "seed", "resources"]:
        environment_manager.copy_tree_with_symlinks(
            source_path / "client/catapult" / folder_name, folder_name
        )
