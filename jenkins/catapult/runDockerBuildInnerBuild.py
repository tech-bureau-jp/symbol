...
class BuildEnvironment:
    ...
    def _prepare_environment_variables(self):
        if self.use_conan:
            ...
            if self.environment_manager.is_windows_platform():
                self.environment_manager.set_env_var(
                    "BOOST_ROOT", "c:/usr/catapult/deps/boost"
                )
                self.environment_manager.set_env_var(
                    "GTEST_ROOT", "c:/usr/catapult/deps/google"
                )
            else:
    ...
    def prepare_conan(self):
        ...
        self.dispatch_subprocess(
            ["conan", "remote", "add", "--force", "nemtech", CONAN_NEMTECH_REMOTE]
        )
...
class BuildManager(BasicBuildManager):
    ...
    def run_cmake(self, source_path, output_path, cmake_preset):
        ...
        if self.environment_manager.is_windows_platform():
            ...
            self.dispatch_subprocess(
                ["cmake"] + cmake_preset + cmake_settings + ["-G", "Ninja", source_path]
            )
    ...
    def build(self):
        if self.environment_manager.is_windows_platform():
            ...
            shutil.copy2(
                "C:/Users/ContainerAdministrator/scoop/apps/ccache/current/ccache.exe",
                "c:/tmp/_build/cl.exe",
            )
    ...
    def copy_dependencies(self, destination):
        ...
        if self.environment_manager.is_windows_platform():
            self.environment_manager.copy_glob_with_symlinks(
                "c:/usr/catapult/deps/boost/lib", "*.dll", destination
            )
            for name in ["facebook", "mongodb", "openssl", "zeromq"]:
                self.environment_manager.copy_glob_with_symlinks(
                    f"c:/usr/catapult/deps/{name}/bin", "*.dll", destination
                )

            for name in ["engines-3", "ossl-modules"]:
                self.environment_manager.copy_tree_with_symlinks(
                    f"c:/usr/catapult/deps/openssl/lib/{name}",
                    Path(destination) / "openssl/lib" / name,
                )
        ...
        for name in [
            ...
            self.environment_manager.copy_glob_with_symlinks(
                "/mybuild/lib", f"libboost_{name}.so*", destination
            )
        ...
        for name in [
            ...
            self.environment_manager.copy_glob_with_symlinks(
                system_bin_path, f"lib{name}.so*", destination
            )
        ...
        for name in ["crypto", "ssl"]:
            self.environment_manager.copy_glob_with_symlinks(
                openssl_source_directory, f"lib{name}.so*", Path(destination)
            )

        for name in ["engines-3", "ossl-modules"]:
            self.environment_manager.copy_tree_with_symlinks(
                openssl_source_directory / name, Path(destination) / name
            )
    ...
    def copy_compiler_deps(self, destination):
        ...
        for dependency_pattern in self.compiler.deps:
            ...
            self.environment_manager.copy_glob_with_symlinks(
                directory_path, pattern, destination
            )
    ...
    def copy_files(self, output_path):
        ...
        if not self.is_release:
            ...
            self.environment_manager.copy_glob_subtree_with_symlinks(
                "./bin", "tests*", tests_output_path
            )
            if EnvironmentManager.is_windows_platform():
                self.environment_manager.copy_glob_subtree_with_symlinks(
                    "./bin", "*.dll", tests_output_path
                )
...
def main():
    ...
    parser.add_argument(
        "--build-configuration", help="path to build configuration yaml", required=True
    )
    parser.add_argument(
        "--dry-run",
    ...
    parser.add_argument(
        "--source-path", help="path to the catapult source code", required=True
    )
    parser.add_argument(
        "--out-dir", help="output path to copy catapult binaries", required=True
    )
    parser.add_argument(
        "--build-type", help="build type for the build", default="RelWithDebInfo"
    )
    ...
    build_path = (
        f"{args.source_path}/_build" if builder.enable_code_coverage else "/tmp/_build"
    )
    ...
    if builder.use_conan:
        ...
        env.run_conan_install(
            args.source_path, conan_options, build_path, args.build_type
        )
        environment_manager.chdir(
            f"{build_path}/build"
            if environment_manager.is_windows_platform()
            else f"{build_path}/build/{args.build_type}"
        )
        conan_preset_name = (
            "conan-default"
            if environment_manager.is_windows_platform()
            else f"conan-{args.build_type.lower()}"
        )
